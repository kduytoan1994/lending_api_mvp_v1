...
module.exports = (app) => {
    ...
    const interest_loan = app.models.interest_loan;
    ...
    app.post('/api/loan/interestPay', (req, res) => {
        ...
        loan.findById(loanId)
            ...
            .then(result => {
                ...
                return interest_loan.find({ where: { loanId: loanId, status: 1 } })
            })
            .then(interestLoanResult => {
                if (interestLoanResult == null || interestLoanResult.length == 0) {
                    loanTemp.status == 2;
                    loanTemp.save(err => {
                        if (err)
                            console.log(err)
